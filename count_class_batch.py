...
class TrackCount(object):
    def __init__(self, model, compound_coef, threshold, nms_threshold, ooi, max_objects,
        ...
        ooi: object of interest str, it can be either "car", "person", "ped_car", "cyclist"
        ...
        print("the objects that need to be counted", self.ooi, self.class_index,
        ...
        # ---------current statistics---------------------------#
        self.current_rois, self.current_std_value, self.current_class_ids, \
            self.framed_metas, self.im = [None for _ in range(5)]
        self.current_person_id = None
        self.new_object_index = None

    def _load_precalculated_stat(self, pre_stat_path):
        _stat = pickle.load(open(pre_stat_path, 'rb'))
        self.pred_stat = _stat # [N, 6] # [image_index, classids, scores, rois], rois: x1, y1, x2, y2
        self.framed_metas = [self.pred_stat["framed_metas"]]

    def _give_prediction_baseon_precalculation(self, im_filename, roi_interest):
        class_ids, _, rois = self.pred_stat[im_filename.split("/")[-1]]
#         imindex = int(im_filename.split('frame_')[1].split('.jpg')[0])
#         _subindex = np.where(self.pred_stat[:, 0] == imindex)[0]
#         class_ids = self.pred_stat[_subindex, 1]
...
#                 class_ids = np.array([], dtype=np.float64)
        im = np.asarray(Image.open(im_filename)) / 255.0
#         im = cv2.imread(im_filename)[:, :, ::-1] / 255.0
    ...
    def give_prediction(self, im_filename, background, roi_interest):
        if len(self.pred_stat) > 0:
            ...
            detector_output = model_arch.get_prediction_ts(im_filename, input_sizes[self.compound_coef],
                                                           self.preprocess_mean, self.preprocess_std,
                                                           background, self.model, self.threshold,
                                                           self.nms_threshold, self.regressBoxes, 
                                                           self.clipBoxes, self.ooi, 
                                                           student=self.student, 
                                                           filter_small_box=self.filter_small_box,
                                                           x_y_threshold=self.x_y_threshold,
                                                           roi_interest=roi_interest)
            [class_ids, scores, rois], framed_metas, im = detector_output
            self.framed_metas = framed_metas
        std_value = tracking_utils.get_std_for_detection(im, rois)
        current_rois, current_std_value = [], []
        ...
        for ci in self.num_class:
            direction_arrow, \
                ...
                percentage_new = count_utils.count_given_boundary(self.current_person_id[ci],
                                                                  self.new_object_index[ci], 
                                                                  self.current_rois[ci],
    ...
    def run(self, im_filenames, save_json_filename, save_video, save_video_filename, show,
        ...
        if use_precalculated_detection != None:
            self._load_precalculated_stat(use_precalculated_detection)
        ...
        while iterr < len(im_filenames) - 1:
            for iterr, single_imname in enumerate(tqdm(im_filenames)):
                ...
                if iterr != 0:
                    _direction_arrow_group, \
                        _current_count_group = self.count_in_separate_window(counted_direction, line_group)
                self.save_stat_in_object(self.current_person_id, _direction_arrow_group, 
                                         self.new_object_index, _ep)
                _direction_arrow_group = self.let_arrow_keep_going(self.current_person_id, 
                                                                   _direction_arrow_group)
                for ci in self.num_class:
                    ...
                    if _cls_name == "person" and np.sum(_current_count_group[ci]) != 0:
                        if type(self.bikespeed) is list or len(np.shape(self.bikespeed)) == 3:
                            ...
                            _count_person, _count_bike, \
                                ...
                                self.direction_string_group[ci], self.speed_group[ci], 
                                _current_count_group[ci], self.bike_id_group, 
                                self.pedestrian_id_group, self.bikespeed)
                        ...
                        _result_per_frame.update(
                            {"aggregate_movement_cars": self.count_movement_group[ci].copy()})
                    fps_track_count += (time.time() - time_init)
        ...
        print("FPS for tracking and counting %.2f" % (len(im_filenames) / fps_track_count))
