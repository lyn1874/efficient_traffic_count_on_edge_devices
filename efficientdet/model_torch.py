...
from efficientnet_torch import EfficientNet as EffNet
from efficientnet_torch.utils import MemoryEfficientSwish, Swish
from efficientnet_torch.utils_extra import Conv2dStaticSamePadding, MaxPool2dStaticSamePadding
...
class RegressorMultiHeads(nn.Module):
    ...
    def __init__(self, in_channels, num_anchors, num_layers, onnx_export=False):
        super(RegressorMultiHeads, self).__init__()
        self.num_layers = num_layers
        self.num_layers = num_layers
        ...
        self.bn_list = nn.ModuleList(
            [nn.ModuleList([nn.BatchNorm2d(in_channels, momentum=0.01, eps=1e-3) for i in range(num_layers)]) for j in
             range(5)])
        self.header = SeparableConvBlock(in_channels, num_anchors * 4, norm=False, activation=False)
        self.header_future = SeparableConvBlock(in_channels, num_anchors * 4, norm=False, activation=False)
        self.swish = MemoryEfficientSwish() if not onnx_export else Swish()
...
class RegressorMultiHeadsBoxLevel(nn.Module):
    ...
    def __init__(self, in_channels, num_anchors, num_layers, onnx_export=False):
        super(RegressorMultiHeadsBoxLevel, self).__init__()
        self.num_layers = num_layers
        self.num_layers = num_layers
        ...
        self.bn_list = nn.ModuleList(
            [nn.ModuleList([nn.BatchNorm2d(in_channels, momentum=0.01, eps=1e-3) for i in range(num_layers)]) for j in
             range(5)])
        self.header = SeparableConvBlock(in_channels, num_anchors * 4, norm=False, activation=False)
        self.header_future = nn.Linear(num_anchors * 4, num_anchors * 4)
        self.swish = MemoryEfficientSwish() if not onnx_export else Swish()
    ...
    def __init__(self, in_channels, num_anchors, num_classes, num_layers, onnx_export=False):
        ...
        self.bn_list = nn.ModuleList(
            [nn.ModuleList([nn.BatchNorm2d(in_channels, momentum=0.01, eps=1e-3) for i in range(num_layers)]) for j in
             range(5)])
        self.header = SeparableConvBlock(in_channels, num_anchors * num_classes, norm=False, activation=False)
        self.header_future = SeparableConvBlock(in_channels, num_anchors * num_classes, norm=False, activation=False)
        self.swish = MemoryEfficientSwish() if not onnx_export else Swish()

    def forward(self, inputs):
        """This inputs only contain feature maps for one time step, and I need to use this 
        input to predict the bbox for the next step
        I will directly work on coordinate level
        """
        feats_group = [[], []]
        for feat, bn_list in zip(inputs, self.bn_list):
            for i, bn, conv in zip(range(self.num_layers), bn_list, self.conv_list):
            ...
            for iterr, _f_ in enumerate([_f_h, _f_f]):
                ...
                _f_ = _f_.contiguous().view(_f_.shape[0], _f_.shape[1], _f_.shape[2], self.num_anchors,
                                            self.num_classes)
                _f_ = _f_.contiguous().view(_f_.shape[0], -1, self.num_classes)
                feats_group[iterr].append(_f_)
        feats_history, feats_future = feats_group
    ...
    def __init__(self, in_channels, num_anchors, num_classes, num_layers, onnx_export=False):
        ...
        self.bn_list = nn.ModuleList(
            [nn.ModuleList([nn.BatchNorm2d(in_channels, momentum=0.01, eps=1e-3) for i in range(num_layers)]) for j in
             range(5)])
        self.header = SeparableConvBlock(in_channels, num_anchors * num_classes, norm=False, activation=False)
        self.header_future = nn.Linear(num_anchors * num_classes, num_anchors * num_classes)
        self.swish = MemoryEfficientSwish() if not onnx_export else Swish()
