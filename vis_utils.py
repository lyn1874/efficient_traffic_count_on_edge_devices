...
def write_to_txt(cam, student, compound_coef, algo, interval, only_person, class_group, subtract_bg, finetune_part,
                 cls_prob, use_feature_maps, use_precalculate, activate_kalman_filter, box_standard,
                 specify_direction=[],
    ...
    _, _a, _count, wrong_prediction = give_input_to_vispostim(path, "aicity", cam + "/", only_person, class_group,
                                                              False, False, None, algo,
                                                              box_standard=box_standard,
                                                              specify_direc=specify_direction,
                                                              return_count=True)
...
def give_required_direction(cam):
    ms = ["move-1-up", "move-1-down", "move-2-up", "move-2-down", "move-3-up", "move-3-down", "move-4-up",
          "move-4-down"]
    if cam == "cam_1" or cam == "cam_1_dawn" or cam == "cam_1_rain":
        ...
        direction_string = np.array(
            ["move-1-down", "move-2-down", "move-3-up", "move-2-up", "move-4-down", "move-1-up"])
    else:
...
def get_path(camera, seq, student, compound_coef, on_off, angle_line, interval, tr_or_tt="train", subtract_bg=False,
             finetune_part="head",
             cls_prob=0.5, use_feature_maps=True, use_precalculate=False, activate_kalman_filter=True, cluster=4,
             camera_specific="specific"):
    ...
    path_shortcut = ["%sDet_student_subtractBG_%s_finetune_%s_clsprob_%.2f" % (camera, subtract_bg,
                                                                               finetune_part,
                                                                               cls_prob) if student else "%sDet_teacher_precalc_%s" % (
    camera,
    use_precalculate)][0]
    ...
    json_path = mom_path_string + 'exp_data/counting_result_%s/%s/track_result_%d_gap%d/' % (
    camera_specific, path_shortcut, compound_coef, interval)
    if not os.path.exists(json_path):
        os.makedirs(json_path)
    json_filename = json_path + "%s_prediction" % seq
    video_outpath = mom_path_string + 'exp_data/counting_result_%s/%s/videos_%d_gap%d/' % (
    camera_specific, path_shortcut, compound_coef, interval)
    txt_outpath = mom_path_string + "exp_data/counting_result_%s/%s/result_txt_%d_gap%d/" % (
    camera_specific, path_shortcut, compound_coef, interval)
